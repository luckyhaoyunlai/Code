...
# pddlFile="pddl1\Subtraction_game\TwoValue\Subtraction-(1,2).pddl"
...
def enumeratePredicate(MaxSize):
                ...
                if li ==2: #add(var,-)
                    for size1 in range(1, li):
                        for choose1 in ExpSet:
                            if choose1['size']==size1 and choose1['Isnum']==False: #修枝 sub第一个参数必须非num
                                    ...
                                    for size2 in range(1,li):
                                        if choose2['size']==size2:
                                            ...
                                            if Goal not in SigSet:
                                                SigSet.append(Goal)
                                                i['outputData']=Goal
                                                ExpSet.append({'Expression':term,'Isnum':False,'outputData':Goal,'size':li})  
    ...
    for i in ExpSet:
        ...
        else:ItemsVar.append(i['Expression'])
        ...
        if i['arity'] == 2:
            for choose1 in ItemsVar:
                for choose2 in Items:
                        tempPredicate = FunExg[i['Function_name']](choose1, choose2)
...
if(Game["var_num"] == 1):
    if(Game["type"] == "normal"):
        position_1[m[X].as_long()] = True  # normal
    else:
        position_1[m[X].as_long()] = False  # misere
if(Game["var_num"] == 2):
    if(Game["type"] == "normal"):
...
try:
    with eventlet.Timeout(600,True):  
        while(True):
            terms = [True, False]
            cover = {}
            preds = []
                ...
                if(s.check() == sat):
                    ...
                    if(s.check() == sat):
                        ...
                        if(s.check() == sat):
                            ...
                            resultFile="result.txt"
                            fp=open(resultFile,'a')
                            fp.write(str(simplify(Not(lossing_formula)))+"\t"+winning_formula_time+"\n")
                            break
            else:
