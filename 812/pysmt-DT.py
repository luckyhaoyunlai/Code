...
pddlFile = r"pddl1\wythoff\_3.3\Wythoff-Game-with-set-K-limit($K-=-{3}$)-.pddl"  # 执行单个pddl
...
while(True):
    if termination_sign:
        ...
        sheet1.write(row, 2, "time-out-3600s")
    ...
    # e = And(X==1,X1==1,X2%3==0,X2<3) #2.7
    # e =Or(And(X2%2==0,(X1-X)%2==0),And(X2%2==1,(X+X1)%2==1))
    e=Or(X!=X1,And((X-1)%7!=0,(X1-2)%7!=0))
    ...
    if str(e) != str(last_e):
        ...
        if check1 == sat:
            ...
            print("unkown con1,cannot judge expression")
            examples = unkownfindstate(ptk)
        else:#如果超过100s判断不出是sat就算做是unsat
            print("sat con1")
            s = Solver()
            s.set('timeout', 6000000)
            s.add(con2)
            check1 = s.check()
            if check1 == sat:
                print("unsat con2")
                ...
                print("unkown con2,cannot judge expression")
                examples = unkownfindstate(ptk)
            else:
                print("sat con2")
                s = Solver()
                ...
                if check1 == sat:
                    ...
                    print("unkown con3,cannot judge expression")
                    examples = unkownfindstate(ptk)
                else:
                    print("sat con3")
                    # losing_formula = e
